CLASS zcl_abapd_customer_gen DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
    INTERFACES if_oo_adt_classrun.
ENDCLASS.

CLASS zcl_abapd_customer_gen IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DELETE FROM zabapd_customer.
    DATA now TYPE timestampl.
    GET TIME STAMP FIELD now.
    DATA(user) = cl_abap_context_info=>get_user_technical_name( ).
    DATA customers TYPE STANDARD TABLE OF zabapd_customer.
    customers = VALUE #(
    ...
    ENDLOOP.
    INSERT zabapd_customer FROM TABLE @customers.
    COMMIT WORK.
    out->write( |Inserted { lines( customers ) } customers into ZABAPD_CUSTOMER.| ).
  ENDMETHOD.
ENDCLASS.
